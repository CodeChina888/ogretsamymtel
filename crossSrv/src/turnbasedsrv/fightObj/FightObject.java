...
import turnbasedsrv.buff.BuffCollection;
...
import turnbasedsrv.support.ParamDefine;
...
public abstract class FightObject implements ISerilizable {
	 ...
	 * @param port
	 */
	public FightObject(FightStageObject stageObj) {
	 ...
	 * @param stageObj
	 */
	public void stageRegister(FightStageObject stageObj) {
		// 在地图中添加活动单元
	...
	public void stageShow() {
		...
		if (Log.fight.isDebugEnabled()) {
			Log.fight.debug("地图单位进入地图: mapSn={}, repSn={}, stageId={}, objId={}, objSn={}, objName={}", stageObj.mapSn,
					stageObj.stageSn, stageObj.stageId, idFight, sn, name);
		}
	...
	public void stageShowRevive() {
		...
		if (Log.fight.isInfoEnabled()) {
			Log.fight.info("地图单位进入地图: stageId={}, objId={}, objName={}", stageObj.stageId, idFight, name);
		}
	 ...
	 * @param stageObj
	 */
	public void stageEnter(FightStageObject stageObj) {
		// if(stageObj.getCell(posNow) == null) {
		// return;
		// }
		// 加入地图并显示
		stageObj.getPort().addQueue(new PortPulseQueue(ParamDefine.stageObj, stageObj, ParamDefine.fightObj, this) {
			@Override
			public void execute(Port port) {
				FightObject fightObj = param.get(ParamDefine.fightObj);
				FightStageObject stageObj = param.get(ParamDefine.stageObj);
				if (fightObj != null) {
					if (stageObj != null) {
	...
	public final void stageLeave() {
		...
		for (TriggerListen listen : this.listenList) {
			this.stageObj.triggerManager.delListen(listen);
		}
		listenList.clear();
		// 设置状态
		inWorld = false;
	...
	public int getFightPos() {
		...
		if (team == ETeamType.Team1) {
			return pos + FightStageObject.FightPosAdd;
		} else {
			return pos;
		}
	}
				...
				for (int i = 0; i < commonSkillList.size(); i++) {
					...
					if (conf != null && conf.type == type && rage >= conf.fireClearRage) {
						...
						if (type == SkillType.Joint.value()) {
							if (!this.stageObj.isExistRelation(conf.relationModelSN))
								continue;
						}
						// 检查是否有控制buff状态
	...
	public boolean addListen(TriggerListen listen) {
		...
		if (!stageObj.triggerManager.addListen(listen)) {
			return false;
		}
