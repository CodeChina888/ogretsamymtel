...
public class Buff {
	...
	public Buff(FightObject owner, int sn, int fightId, FightObject fireObj, Prop fireProp, Prop targetProp,
		...
		if (confBuff == null) {
			Log.fight.error("ConfBuff配表错误，no find sn ={}", sn);
		}
	...
	private void init() {
		...
		for (int i = 0; i < this.confBuff.triggerEffects.length; i++) {
			...
			if (confEffect == null) {
				Log.fight.error("ConfBuff配表错误，no effect sn ={}", effectSn);
				continue;
			}
	...
	public void orderTrigger() {
		...
		// 跳过战斗，不发消息
		if (owner.stageObj.isQuickFight) {
			return;
		}
		Log.fight.info("触发buff：fightObjPos={},sn={},stage:{}", this.owner.getFightPos(), this.sn,
				this.owner.stageObj.stageId);
		owner.stageObj.sendBuffTriggerInfoMsg(data, false);
	}
	...
	public void passiveTrigger(Trigger trigger) {
		...
		Log.fight.info("触发buff被动：fightObjPos={},sn={},stage:{}", this.owner.getFightPos(), this.sn,
				this.owner.stageObj.stageId);
		owner.stageObj.sendBuffTriggerInfoMsg(data, false);
	}
