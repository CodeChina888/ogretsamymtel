...
import turnbasedsrv.stageEvent.EventCastSkill;
...
public class Skill {
	 ...
	 * @return
	 */
	public static Skill newInstance(int sn, int lv, FightObject owner) {
		ConfSkill confSkill = ConfSkill.get(sn);
		if (confSkill == null) {
			Log.fight.error("confSkill配表错误，no find sn ={}", sn);
			return null;
		}
		return new Skill(sn, lv, confSkill, owner);
	}

	/** 构造函数 **/
	private Skill(int sn, int lv, ConfSkill confSkill, FightObject owner) {
		this.sn = sn;
		this.lv = lv;
	...
	public void startPassive() {
		...
		for (Map.Entry<Integer, List<Integer>> entry : this.effectMap.entrySet()) {
			...
			this.owner.stageObj.triggerManager.addListen(listen);
	...
	public void passiveTrigger(Trigger trigger) {
		...
		// 跳过战斗，不触发等待事件
		if (this.owner.stageObj.isQuickFight) {
			return;
		}
		Log.fight.info("触发被动技能：fightObjPos={},sn={},stage:{}", this.owner.getFightPos(), this.sn,
				this.owner.stageObj.stageId);
		this.owner.stageObj.sendCastSkillInfoMsg(data);
	}
	...
	public void cast() {
		...
		this.owner.stageObj.triggerManager.addTrigger(triggerAfterAttack);
		// 普攻后，怒攻后
		if (this.confSkill.type == SkillType.Common.value()) {// 普攻
			...
			this.owner.stageObj.triggerManager.addTrigger(trigger);
		} else {// 怒攻
			TriggerParam param = new TriggerParam(data, this.owner);
		...
		for (FightObject target : allTargetList) {
			...
			this.owner.stageObj.triggerManager.addTrigger(triggerAfterAttacked);
			// 被普攻后，被怒攻后
			if (this.confSkill.type == SkillType.Common.value()) {// 普攻
			...
			if (this.confSkill.atkType == AtkType.Cure.value()) {
				...
				this.owner.stageObj.triggerManager.addTrigger(trigger1);
			}
		}
		...
		if (confSkill.stiffTime > 0) {
			// long lastTime = (long)
			// (confSkill.stiffTime/(double)this.owner.stageObj.speedTimes);
			long lastTime = (long) (confSkill.stiffTime);
			this.owner.stageObj.addStageEvent(new EventCastSkill(lastTime));
		}
		Log.fight.info("使用主动技能：fightObjPos={},sn={},stage:{}", this.owner.getFightPos(), this.sn,
				this.owner.stageObj.stageId);
		this.owner.stageObj.sendCastSkillInfoMsg(data);
	}
	...
	public void killed() {
		for (TriggerListen listen : this.listenList) {
			this.owner.stageObj.triggerManager.delListen(listen);
		}
