...
import game.worldsrv.config.ConfActivitySingleRechargeEveryday;
 ...
 *
 */
public class ActivitySingleRechargeEveryday extends ActivityTypeBase {
	private static final int type = ActivityTypeDefine.Activity_TYPE_21;	//类型
	private static final ActivitySingleRechargeEveryday instance = new ActivitySingleRechargeEveryday();	//实例
	
	public static final String RECHARGE = "recharge";
	public static final String COUNT = "count";
	public static final String GETNUM = "getNum";
	
	private ActivitySingleRechargeEveryday() {
	}
	...
	public static ActivityTypeBase getInstance(){
		return ActivitySingleRechargeEveryday.instance;
	}
	 ...
	 */
	public int getType(){
		return ActivitySingleRechargeEveryday.type;
	}
	...
	public List<ActivityZoneItemObject> initOperateParam(ActivityObject activity,String paramStr){
		...
		Collection<ConfActivitySingleRechargeEveryday>  confList = ConfActivitySingleRechargeEveryday.findAll();
		for(ConfActivitySingleRechargeEveryday conf:confList){
	...
	public Param getShowParam(ActivityObject activity, HumanObject humanObj, List<DActivityZoneItem> zoneList){
		...
		for (Map.Entry<Integer, List<ActivityZoneItemObject>> entry : activity.zoneItems.entrySet()) {
			...
			if(activityHumanDataMap != null && activityHumanDataMap.size()>0){
				...
				//日期是同一天
				if(human_data.getNumValue() == Utils.longValue(ActivityTypeBase.getNowDateValue())){
					...
					if(jodata != null){
						...
						if(count > getNum && rechargeCount >getNum){
							...
							if(count - getNum > 0 && getNum >0){
								buttonStatus = EAwardType.AwardNot_VALUE;
							}
	...
	public boolean commitActivity(ActivityObject activity, HumanObject humanObj, List<ActivityParamObject> paramList){
		...
		snList.add(0);
		numValues.put(0, Utils.longValue(ActivityTypeBase.getNowDateValue()));
