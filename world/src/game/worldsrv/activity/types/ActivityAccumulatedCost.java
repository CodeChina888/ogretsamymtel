 ...
 *
 */
public class ActivityAccumulatedCost extends ActivityTypeBase {
	private static final int type = ActivityTypeDefine.Activity_TYPE_11;	//类型
	private static final ActivityAccumulatedCost instance = new ActivityAccumulatedCost();//实例	
	
	private static final String jsonKey_Gold = "Gold";
	private static final String jsonKey_AlreadyGet = "AlreadyGet";
	
	private ActivityAccumulatedCost() { 
	}
	/**
	 * 获取单例
	 * @return
	...
	public static ActivityTypeBase getInstance(){
		return ActivityAccumulatedCost.instance;
	}
	 ...
	 */
	public int getType(){
		return ActivityAccumulatedCost.type;
	}
	...
	public List<ActivityZoneItemObject> initOperateParam(ActivityObject activity,String paramStr){
		...
		Collection<ConfActivityAccumulatedCost> confAll = ConfActivityAccumulatedCost.findBy("activityId",activity.id);
		List<ActivityZoneItemObject> zoneItems = new ArrayList<>();
		for(ConfActivityAccumulatedCost conf:confAll){
			int aid = conf.sn;
