 ...
 * @author shenjh
 */
public class StageObjectInstMoba extends StageObjectInstTeam {

	private int winTeamId = 0;// 赢的队伍ID：0代表平局，>0代表赢的队伍ID

	public StageObjectInstMoba(StagePort port, long stageId, int stageSn, int mapSn, boolean isMonsterAddProp) {
	...
	/**
	 * 获取赢的队伍ID：判断双方的终极BOSS是否死亡，只要有一方的终极BOSS死亡则对方胜利，否则时间到了算平局
	 * @return
	 */
	private int getWinTeamId() {
		int winTeamId = 0;// 0表示平局
		Map<Integer, List<HumanObject>> mapTeamHumanObj = this.getTeamHumanObj();
		// 判断双方的终极BOSS是否死亡，只要有一方的终极BOSS死亡则对方胜利，否则时间到了算平局
		if (mapTeamHumanObj.size() == 2) {// 2个队伍
			//for (Entry<Integer, List<HumanObject>> entry : mapTeamHumanObj.entrySet()) {
				// Utils.intValue(entry.getKey());
				// entry.getValue()
			//}
		}
		return winTeamId;
