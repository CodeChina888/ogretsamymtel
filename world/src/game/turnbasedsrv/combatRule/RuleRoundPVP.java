...
public class RuleRoundPVP extends CombatRuleBase {
	 ...
	 * @param value
	 */
	public RuleRoundPVP(int id, String value) {
		super(id);
	}
	...
	public String getType() {
		return CombatRuleDefine.RoundPVP;
	}
	...
	private boolean castCommonSkill(CombatObject stageObj) {
		...
		if (stageObj.orderList.size() > 0) {
			...
			if (fightObj.isHuman() && !stageObj.autoFightMap.get(fightObj.team) && fightObj.canCastCommonSkill()) {
				// 设置战斗暂停
				stageObj.stopFight(true, ParamManager.PVPHumanCastTime, fightObj.idFight);
			} else {
