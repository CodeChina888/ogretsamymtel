...
public class CombatObjectInstRes extends CombatObject {
	private HumanMirrorObject humanMirrorObj;// 挑战者镜像
	private InstanceParam instanceParam;// 额外数据
	
	/** 敌方初始总血量 **/
	public long totalHp = 0;

	
	public CombatObjectInstRes(Port port, StageObject mapStageObj, int stageSn, int mapSn, 
			int fightType) {
	...
	public CombatObjectInstRes(FightRecord fightRecord, Port port, StageObject mapStageObj, 
	...
	public void initFightObjOverride() {
		if (fightType == ECrossFightType.FIGHT_INST_RES_VALUE) {
		...
		// 敌方总血量
		totalHp = getTeamTotalMaxHp(ETeamType.Team2);
	...
	public SCTurnbasedFinish getFinishMsg() {
		...
		// 造成的伤害总和
		Integer damageRatio = Utils.getParamValue(exParam, JsonKey.InstPercent, 0);
		// 万分比值
		damageRatio *= Utils.I100;
		msg.addParam32(damageRatio);
		return msg.build();
	}
