...
public class CombatObjectCompete extends CombatObject {
	private HumanMirrorObject humanMirrorObj;// 挑战者镜像
	private CompeteParam competeParam;// 额外数据
	private Param param;
	
	public CombatObjectCompete(Port port, StageObject mapStageObj, int stageSn, int mapSn, 
		...
		// 提取额外数据
		this.param = param;
		this.competeParam = Utils.getParamValue(this.param, HumanMirrorObject.CompeteParam, null);
		if (null == this.competeParam) {
			Log.fight.error("没有额外数据！error in CompeteParam is null");
		}
	}

	public CombatObjectCompete(FightRecord fightRecord, Port port, StageObject mapStageObj, 
	...
	public void initFightObjOverride() {
		if (fightType == ECrossFightType.FIGHT_COMPETE_VALUE) {
