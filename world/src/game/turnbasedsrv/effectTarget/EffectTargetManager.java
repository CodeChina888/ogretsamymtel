...
public class EffectTargetManager extends ManagerBase {
	...
	private List<FightObject> getFriendRand(ConfSkillEffect confEffect, SourceParam source, ProcessParam process) {
		...
		CombatObject combatObj = source.combatObj;
		if (combatObj == null) {
			Log.fight.error("no find combatObj!");
			return ret;
		}
		// 取出所有友方后再过滤
		List<FightObject> list = getFriendObjList(source, process);
		// 取随机数量的对象
		ret = getRandObj(combatObj.randUtils, count, list);
		return ret;
	}
	...
	private List<FightObject> getFriendRandOther(ConfSkillEffect confEffect, SourceParam source, ProcessParam process) {
		...
		// 他人优先
		for (FightObject obj : combatObj.getFightObjs().values()) {
	...
	private List<FightObject> getEnemyPropHighest(ConfSkillEffect confEffect, SourceParam source,
		...
		if (null == rule) {
			Log.fight.info("取目标规则错误：targetParam1={},sn={}", confEffect.targetParam1, confEffect.sn);
			return ret;
		}
		FightObject fightObj = source.fightObj;
		if (null == fightObj) {
	...
	private List<FightObject> getCaster(ConfSkillEffect confEffect, SourceParam source, ProcessParam process) {
		...
		if (rule == EffectTargetRule.One) {// 单人
			...
			CombatObject combatObj = source.combatObj;
			List<FightObject> list = getFriendObjList(combatObj, casterObj);
			// 取符合指定规则的目标
	...
	private List<FightObject> getBuffOwner(ConfSkillEffect confEffect, SourceParam source, ProcessParam process) {
		...
		if (rule == EffectTargetRule.One) {// 单人
			...
			CombatObject combatObj = source.combatObj;
			List<FightObject> list = getFriendObjList(combatObj, buffOwnerObj);
			// 取符合指定规则的目标
	...
	private List<FightObject> getTriggerSelf(ConfSkillEffect confEffect, SourceParam source, ProcessParam process) {
		...
		if (rule == EffectTargetRule.One) {// 单人
			...
			CombatObject combatObj = source.combatObj;
			List<FightObject> list = getFriendObjList(combatObj, triggerSelfObj);
			// 取符合指定规则的目标
	...
	private List<FightObject> getTriggerTarget(ConfSkillEffect confEffect, SourceParam source, ProcessParam process) {
		...
		if (rule == EffectTargetRule.One) {// 单人
			...
			CombatObject combatObj = source.combatObj;
			List<FightObject> list = getFriendObjList(combatObj, triggerTargetObj);
			// 取符合指定规则的目标
	...
	private List<FightObject> getEnemyObjList(SourceParam source, ProcessParam process) {
		...
		ret = getEnemyObjList(combatObj, fightObj);
		return ret;
	}

	private List<FightObject> getEnemyObjList(CombatObject combatObj, FightObject fightObj) {
		List<FightObject> ret = new ArrayList<>();
		int count = combatObj.getFightObjCount();
		if (count <= 0) {
			return ret;
		}
		for (FightObject obj : combatObj.getFightObjs().values()) {
	...
	private List<FightObject> getFriendObjList(SourceParam source, ProcessParam process) {
		...
		ret = getFriendObjList(combatObj, fightObj);
		return ret;
	}

	private List<FightObject> getFriendObjList(CombatObject combatObj, FightObject fightObj) {
		List<FightObject> ret = new ArrayList<>();
		if (null == combatObj || null == fightObj) {
			return ret;
		}
		int count = combatObj.getFightObjCount();
		if (count <= 0) {
	...
	private FightObject getTriggerSelfObj(SourceParam source, ProcessParam process) {
		...
		Trigger trigger = combatObj.triggerManager.nowTrigger;
		if (trigger == null) {
	...
	private List<FightObject> getByRule(EffectTargetRule rule, List<FightObject> list, FightObject fightObj) {
		...
		case One: { // 单人
			...
			break;
		case RandomOne: { // 随机单人
			int rand = fightObj.combatObj.randUtils.nextInt(list.size());
		...
		case BackRowOne: { // 后排单人
			...
			break;
		case Col2: { // 随机2列
			int[] posFind = findPosFromCol(2, fightObj.combatObj.randUtils);
		...
		case Triangle: { // 三角范围
			...
			break;
		case Male: { // 男性对象
			List<FightObject> objFind = getFightObjListBySex(list, 1);
			if (!objFind.isEmpty()) {
				ret.addAll(objFind);
			}
		}
			break;
		case Female: { // 女性对象
			List<FightObject> objFind = getFightObjListBySex(list, 2);
			if (!objFind.isEmpty()) {
				ret.addAll(objFind);
			}
		}
	 ...
	 * @param sex 性别 男1，女2
	 */
	private List<FightObject> getFightObjListBySex(List<FightObject> list, int sex) {
		List<FightObject> ret = new ArrayList<>();
		for (FightObject obj : list) {
			if (obj != null && obj.sex == sex) {
				ret.add(obj);
			}
		}
		return ret;
