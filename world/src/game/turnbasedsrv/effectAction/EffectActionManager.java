package game.turnbasedsrv.effectAction;
...
public class EffectActionManager extends ManagerBase {
	...
	public void doEffectAction(ConfSkillEffect confEffect, SourceParam source, ProcessParam process) {
		...
		case BuffAdd: { // 给目标增加buff
			...
			break;
		case BuffAddStun: {	
			doBuffAddStun(confEffect, source, process);
		}
			break;
		...
		case RageAdd: { // 给目标加怒
			...
			break;
		case DamAddEx: { // 给目标增加根据属性换算来的最终伤害值
			doDamAddEx(confEffect, source, process);
		}	
			break;
		...
		default: {
			Log.logError(Log.fight,"未处理的逻辑类型：actionType={},sn={}", confEffect.actionType, confEffect.sn);
		}
		}
	}
	...
	private void operateHurt(ConfSkillEffect confEffect, SourceParam source, ProcessParam process) {
		...
		SkillCastData skillData = getSkillCastData(source, process);
		// 技能养成加成值
		valuePct += skillData.skill.power;
		value += skillData.skill.value;
		
		// 攻击者必中属性
		ValueBase certainlyHit = attackerProp.getOnePropValue(FightPropName.CertainlyHit.value());
		// 必中属性=false，则正常计算，否则必中，无视丢失闪避无敌
		if (!certainlyHit.getBooleanValue()) {
			// 判断是否闪避
			...
			double hitValue = hit.getNumberValue() + ParamManager.hitParam - dodge.getNumberValue();
			if (hitValue < 2000) {
				hitValue = 2000;
			}
	...
	private void finalDamage(SourceParam source, ProcessParam process, SkillCastData skillData, FightObject target,
		...
		if (damBack != null && damBack.isNumberValue() && damBack.getNumberValue() > 0) {
			...
			// 受击者的5倍攻击力
			int targetAtk = (int) targetProp.getOnePropValue(FightPropName.Atk.value()).getNumberValue() * 5;
			// 伤害超过受击者攻击力5倍，则伤害等于受击者攻击力
			if (hurtBack > targetAtk) {
				hurtBack = targetAtk;
			}
		...
		ValueBase certainlyHit = attackerProp.getOnePropValue(FightPropName.CertainlyHit.value());
		// 回光返照状态
		ValueBase borrowedTime = targetProp.getOnePropValue(FightPropName.BorrowedTime.value());
		if (!certainlyHit.getBooleanValue() &&  // 施法者不是必定命中
				(borrowedTime != null && borrowedTime.isNumberValue() && borrowedTime.getNumberValue() > 0) ) {
			// 回光返照
			leftHurt = (int) (leftHurt * (double)borrowedTime.getNumberValue() / Utils.D10000);
		} else {
			leftHurt = -leftHurt;
		}
		// 最终伤害
	...
	private void operateCure(ConfSkillEffect confEffect, SourceParam source, ProcessParam process) {
		...
		// 治疗值=（治疗技能百分比*施法者攻击力+固定治疗值）*（100%+治疗率+被治疗者的被治疗率）+施法者治疗值+被治疗者的被治疗值
		int cureValue = (int) ( (valuePct / Utils.D10000 * atk + value)
				* (Utils.D10000 + cureAdd.getNumberValue() + healAdd.getNumberValue()) / Utils.D10000 
	...
	private void operatePoison(ConfSkillEffect confEffect, SourceParam source, ProcessParam process) {
		...
		int poisonValue = (int) (valuePct/Utils.D10000
				*(Utils.D10000+poisonAdd.getNumberValue()+antiPoisonAdd.getNumberValue())/Utils.D10000*atk
				+poisonAddEx.getNumberValue()+antiPoisonAddEx.getNumberValue()) * buffData.buff.times; // 叠加buff时重新计算叠加属性加成
		if(poisonValue < 0) {
			poisonValue = 0;
		} else {
		...
		double damageCeof = (attackerProp.getOnePropValue(FightPropName.DamCeof.value()).getNumberValue()
				-targetProp.getOnePropValue(FightPropName.DamRedCeof.value()).getNumberValue()+Utils.D10000)/Utils.D10000;
	...
	private void operateBlood(ConfSkillEffect confEffect, SourceParam source, ProcessParam process) {
		...
		int bloodValue = (int) (valuePct/Utils.D10000
				*(Utils.D10000+bloodAdd.getNumberValue()+antiBloodAdd.getNumberValue())/Utils.D10000*atk
				+bloodAddEx.getNumberValue()+antiBloodAddEx.getNumberValue()) * buffData.buff.times; // 叠加buff时重新计算叠加属性加成
		if(bloodValue < 0) {
			bloodValue = 0;
		} else {
	...
	private void operateBurn(ConfSkillEffect confEffect, SourceParam source, ProcessParam process) {
		...
		int burnValue = (int) (valuePct/Utils.D10000
				*(Utils.D10000+burnAdd.getNumberValue()+antiBurnAdd.getNumberValue())/Utils.D10000*atk
				+burnAddEx.getNumberValue()+antiBurnAddEx.getNumberValue()) * buffData.buff.times; // 叠加buff时重新计算叠加属性加成
		if(burnValue < 0){
			burnValue = 0;
		} else {
			burnValue = -burnValue;
	...
	private void buffAdd(ConfSkillEffect confEffect, SourceParam source, ProcessParam process) {
		...
		// 旧的属性
		Prop oldTargetProp = process.targetProp;
		// 新的属性
		Prop curTargetProp = PropManager.inst().getCheckAttackerPropCopy(targetObj, creator, source.skillData);
		// 血量变化值
		int hpMaxValue = curTargetProp.getMaxHp() - oldTargetProp.getMaxHp();
		if (hpMaxValue != 0) {
			int addHp = PropManager.inst().addHp(targetObj, hpMaxValue);
			addTargetDataDamage(source, process, targetObj, addHp);
		}
	}
	 ...
	 * 给目标增加眩晕buff
	 */
	private void doBuffAddStun(ConfSkillEffect confEffect, SourceParam source, ProcessParam process) {
		List<FightObject> targetList = process.targetObjList;
		if (targetList == null) {
			return;
		}
		...
		for (FightObject obj : targetList) {
			...
			process.targetProp = targetProp;
			// 执行行为逻辑
			buffAddStun(confEffect, source, process);
		}
	}
	 ...
	 * 给目标增加buff
	 */
	private void buffAddStun(ConfSkillEffect confEffect, SourceParam source, ProcessParam process) {
		CombatObject stageObj = source.combatObj;
		if (stageObj == null) {
			Log.fight.error("===error in stageObj is null");
			return;
		...
		Prop attackerProp = process.attackerProp; // 释放技能的对象属性
		Prop targetProp = process.targetProp;	// 受击对象的属性
		if (targetProp == null) {
			Log.fight.error("===缺少targetProp");
			return;
		}
		// 攻击者必控属性
		ValueBase certainlyControl = attackerProp.getOnePropValue(FightPropName.CertainlyControl.value());
		// 目标必被控制属性
		ValueBase weak = targetProp.getOnePropValue(FightPropName.Weak.value());
		
		// 必控属性=false 与 必被控属性=false，正常计算控制几率
		if (!certainlyControl.getBooleanValue() && !weak.getBooleanValue()) {
			ValueBase control = attackerProp.getOnePropValue(FightPropName.Control.value());
			ValueBase antiControl = targetProp.getOnePropValue(FightPropName.AntiControl.value());
			// 控制几率
			int controlOdds = (int)( (Math.pow(ParamManager.controlParamG, 
									(control.getNumberValue() - antiControl.getNumberValue()) / (double)ParamManager.controlParamH ) 
									* ParamManager.controlParamI) * Utils.D10000 );
			// 上下限控制
	    	if (controlOdds < ParamManager.controlParamMin) {
	    		controlOdds = ParamManager.controlParamMin;
	    	}
	    	if (controlOdds > ParamManager.controlParamMax) {
	    		controlOdds = ParamManager.controlParamMax;
	    	}
	    	// 眩晕未命中
			if (stageObj.randUtils.nextInt(Utils.I10000) > controlOdds) {
				return;
			}
		}
	...
	private void buffRemoveBySn(ConfSkillEffect confEffect, SourceParam source, ProcessParam process) {
		...
		// 处理驱散，净化buffList
		targetObj.buffManager.excuteInvalidBuff(buffList, false);
	}
	...
	private void rageAdd(ConfSkillEffect confEffect, SourceParam source, ProcessParam process) {
		...
		addTargetDataValue(source, process, targetObj, SkillTargetData.ValueRage, rageChange);
	}
	
	/**
	 * 给目标增加根据属性换算来的最终伤害值
	 */
	private void doDamAddEx(ConfSkillEffect confEffect, SourceParam source, ProcessParam process) {
		List<FightObject> targetList = process.targetObjList;
		if (targetList == null) {
			return;
		}
		...
		for (FightObject obj : targetList) {
			process.targetObj = obj;
			// 执行行为逻辑
			damAddEx(confEffect, source, process);
		}
	}
	/**
	 * 给目标增加根据属性换算来的最终伤害值
	 */
	private void damAddEx(ConfSkillEffect confEffect, SourceParam source, ProcessParam process) {
		CombatObject stageObj = source.combatObj;
		if (stageObj == null) {
			Log.fight.error("===error in stageObj is null");
			return;
		...
		// 获取配表值
		int ratio = Utils.intValue(confEffect.actionParam1); // 万分比
		String propName = confEffect.actionParam2; // 加成额外最终伤害的属性
		int damAddEx = (int) (ratio * PropManager.inst().getFinalPropValue(propName, targetObj).getNumberValue() / Utils.D10000);
		// 增加额外附加伤害
		PropManager.inst().addDamAddEx(targetObj, damAddEx);
	}
		...
		if (valueMap != null && valueMap.containsKey(SkillTargetData.ValueDamage)) {
			...
			if (damage < 0) {
				...
				if (PropManager.inst().getCurHp(target) <= 0) {
					...
					target.Deading(killer);
					// 添加击杀人数
					if (skillData != null) {
						skillData.addKillCount();
					}
				}
			}
		}
		//Log.logTest(Log.fight,"目标数据:targetPos:{},hp:{},skillSn:{},valueMap:{},state:{},buffList:{},isBuff:{}",
