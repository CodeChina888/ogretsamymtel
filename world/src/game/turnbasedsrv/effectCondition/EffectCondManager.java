...
public class EffectCondManager extends ManagerBase {
	...
	private int getCondValue(EffectCondType type, ConfSkillEffect confEffect, SourceParam source,
		...
		case SelfPropPct: { // 自身某属性万分比值 vk都是万分比填法
			ret = getSelfProp(false, confEffect, source, process);
		}
			break;
		case TargetPropPct: { // 目标某属性万分比值 vk都是万分比填法
			ret = getTargetProp(false, confEffect, source, process);
		}
			break;
		case TriggerSelfPropPct: { // 触发者某属性万分比值 vk都是万分比填法
			ret = getTriggerSelfProp(false, confEffect, source, process);
		}
			break;
		case TriggerTargetPropPct: { // 触发目标某属性万分比值 vk都是万分比填法
			ret = getTriggerTargetProp(false, confEffect, source, process);
		}
			break;

		case SelfCountry: { // 自身派别
			ret = getSelfCountry(confEffect, source, process);
		}
			break;
		case TargetCountry: { // 目标派别
			ret = getTargetCountry(confEffect, source, process);
		}
			break;
		case TriggerSelfCountry: { // 获取触发者派别
			ret = getTriggerSelfCountry(confEffect, source, process);
		}
			break;
		case TriggerTargetCountry: { // 获取触发目标派别
			ret = getTriggerTargetCountry(confEffect, source, process);
		}
			break;
		...
		case TriggerTargetHasBuffType: { // 触发目标含有某类型buff
			...
			break;
		case SkillKillCount: { // 本次技能击杀的人数
			ret = getSkillKillCount(confEffect, source, process);
		}
            break;
		default: {
	...
	private int getSelfProp(boolean isPct, ConfSkillEffect confEffect, SourceParam source, ProcessParam process) {
		...
		// 都是万分比填法，不需要返回除以10000.0的值，并且转为int类型，只会出现0和1
		if (isPct) {
	...
	private int getSelfCountry(ConfSkillEffect confEffect, SourceParam source, ProcessParam process) {
		int ret = 0;
		FightObject fightObj = source.fightObj;
		if (fightObj == null) {
			Log.fight.error("no find fightObj！conditionType={},sn={}", confEffect.conditionType, confEffect.sn);
	...
	private int getTargetCountry(ConfSkillEffect confEffect, SourceParam source, ProcessParam process) {
		int ret = 0;
		FightObject targetObj = process.targetObj;
		if (targetObj == null) {
			// 没有目标，则检查目标列表有没有
	...
	private int getTriggerSelfCountry(ConfSkillEffect confEffect, SourceParam source, ProcessParam process) {
		int ret = 0;
		CombatObject stageObj = source.combatObj;
		if (stageObj == null) {
			Log.fight.error("no find stageObj！conditionType={},sn={}", confEffect.conditionType, confEffect.sn);
	...
	private int getTriggerTargetCountry(ConfSkillEffect confEffect, SourceParam source, ProcessParam process) {
		int ret = 0;
		CombatObject stageObj = source.combatObj;
		if (stageObj == null) {
			Log.fight.error("no find stageObj！conditionType={},sn={}", confEffect.conditionType, confEffect.sn);
	...
	private int getSkillKillCount(ConfSkillEffect confEffect, SourceParam source, ProcessParam process) {
		int ret = 0;
		CombatObject stageObj = source.combatObj;
		if (stageObj == null) {
			Log.fight.error("no find stageObj！conditionType={},sn={}", confEffect.conditionType, confEffect.sn);
